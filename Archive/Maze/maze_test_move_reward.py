...
class Network(nn.Module):
    ...
    def __init__(self, isize, hsize):
        ...
        self.hebb_mult = torch.zeros([hsize, hsize], dtype=torch.int32, device="cuda")
        self.perc_not_froze = 100  # 1/perc_not_froze will be usable
        for i in range(hsize):
            for j in range(hsize):
                if (j - i) % self.perc_not_froze == 0:
                    self.hebb_mult[i, j] = 1
...
def train(paramdict):
    ...
    #net.load_state_dict(torch.load('200k_5by5_limitted_heb.dat'))
    #net.load_state_dict(torch.load('2k_5by5_1hebb.dat'))
    #net.load_state_dict(torch.load('50k_trained_uncertainty.dat'))
    #net.load_state_dict(torch.load('Fullsize_Limithebb.dat'))
    ...
    for nb in range(BATCHSIZE):
        ...
        reward_move_time[nb] = random.sample(range(movebuffer, params['eplen'] - movebuffer), NUMMOVES)
    ...
    for numstep in range(params['eplen']):
        ...
        labg = lab.copy()
        for nb in range(BATCHSIZE):
            if numstep in reward_move_time[nb] and move:
                myrposr = 0;
                myrposc = 0
                while lab[myrposr, myrposc] == 1 or (myrposr == CTR and myrposc == CTR):
                    myrposr = np.random.randint(1, LABSIZE - 1)
        ...
        #hebbian_trace.append(hebb[0][0].detach().numpy())
        writer.add_histogram("Hebbian Terms", hebb, numstep)
        ...
        for nb in range(BATCHSIZE):
            ...
            if rposr[nb] == posr[nb] and rposc[nb] == posc[nb]:
                ...
                while lab[posr[nb], posc[nb]] == 1 or (rposr[nb] == posr[nb] and rposc[nb] == posc[nb]):
                    posr[nb] = np.random.randint(1, LABSIZE - 1)
                    posc[nb] = np.random.randint(1, LABSIZE - 1)
            if reward_move_time[nb][0] < numstep:
    ...
    print("Done!")
    writer.flush()
    #np.savetxt("foo.csv", hebbian_trace, delimiter=",")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eplen", type=int, help="length of episodes", default=400)
