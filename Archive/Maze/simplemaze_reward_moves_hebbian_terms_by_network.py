...
# simple maze but hebbian terms are learned from our network
...
class Network(nn.Module):
    ...
    def forward(self, inputs,hidden, movetime, currenttime, BATCHSIZE,hnet): # hidden is a tuple containing the h-state (i.e. the recurrent hidden state) and the hebbian trace
            HS = self.hsize
...
def train(paramdict):
    ...
    #net.load_state_dict(torch.load('200k_trained.dat'))
    ...
    for numiter in range(params['nbiter']):
        ...
        for numstep in range(params['eplen']):
            ...
            ## Running the network
            y, v, (hidden, hebb) = net(inputsC, (hidden, hebb), reward_move_time, numiter, BATCHSIZE, hnet)  # y  should output raw scores, not probas
        ...
        if (numiter+1) % params['save_every'] == 0:
            ...
            with open('grad_movereward_'+suffix+'.txt', 'w') as thefile:
                for item in all_grad_norms[::10]:
                        thefile.write("%s\n" % item)
            with open('loss_movereward_'+suffix+'.txt', 'w') as thefile:
                for item in all_total_rewards[::10]:
                        thefile.write("%s\n" % item)
            torch.save(net.state_dict(), 'torchmodel_movereward_'+suffix+'.dat')
            with open('params_movereward_'+suffix+'.dat', 'wb') as fo:
                pickle.dump(params, fo)
            if os.path.isdir('/mnt/share/tmiconi'):
                print("Transferring to NFS storage...")
                for fn in ['params_movereward_'+suffix+'.dat', 'loss_'+suffix+'.txt', 'torchmodel_movereward_'+suffix+'.dat']:
                    result = os.system(
