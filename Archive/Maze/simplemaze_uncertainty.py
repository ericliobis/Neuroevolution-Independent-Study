...
# simple maze but doesn't move reward. Each observation has some uncertainty
import argparse
import pdb
...
def train(paramdict):
    ...
    UNCERTAINTY = 0.05 # Percent uncertainty. (percent chance that each observation is changed)
    ...
    for numiter in range(params['nbiter']):
        ...
        for numstep in range(params['eplen']):
            ...
            for nb in range(BATCHSIZE):
                ...
                #add the uncertainty to the observations
                for i in range(RFSIZE * RFSIZE):
                    if random.uniform(0,1) < UNCERTAINTY:
                        inputs[nb, i] = 1 - inputs[nb, i]
        ...
        if (numiter+1) % params['save_every'] == 0:
            ...
            with open('grad_uncert_'+suffix+'.txt', 'w') as thefile:
                for item in all_grad_norms[::10]:
                        thefile.write("%s\n" % item)
            with open('loss_uncert_'+suffix+'.txt', 'w') as thefile:
                for item in all_total_rewards[::10]:
                        thefile.write("%s\n" % item)
            torch.save(net.state_dict(), 'torchmodel_'+suffix+'.dat')
            with open('params_uncert_'+suffix+'.dat', 'wb') as fo:
                pickle.dump(params, fo)
            if os.path.isdir('/mnt/share/tmiconi'):
                print("Transferring to NFS storage...")
                for fn in ['params_uncert_'+suffix+'.dat', 'loss_'+suffix+'.txt', 'torchmodel_'+suffix+'.dat']:
                    result = os.system(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nbiter", type=int, help="number of learning cycles", default=50000)
