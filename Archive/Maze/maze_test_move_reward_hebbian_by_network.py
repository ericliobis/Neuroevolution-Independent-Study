...
#Goal: Output the Hebbian terms for a lot of samples
...
class Network(nn.Module):
    ...
    def forward(self, inputs, hidden, movetime, currenttime, BATCHSIZE,
                hnet):  # hidden is a tuple containing the h-state (i.e. the recurrent hidden state) and the hebbian trace
        HS = self.hsize
        ...
        X = torch.zeros([1, 3], dtype=torch.int32, device='cuda')
        y = torch.zeros([BATCHSIZE, self.hsize, self.hsize], dtype=torch.int32, device='cuda')
...
class HebbNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(input1, hidden)
...
def train(paramdict):
    ...
    hnet = HebbNet().to(device)
    hnet.load_state_dict(torch.load('Hebbian_analysis.dat'))
    ...
    for nb in range(BATCHSIZE):
        ...
        reward_move_time[nb] = nb ;#random.sample(range(movebuffer, params['eplen'] - movebuffer), NUMMOVES)
    ...
    for numstep in range(params['eplen']):
        ...
        labg = lab.copy()
        for nb in range(BATCHSIZE):
            if numstep == reward_move_time[nb]+movebuffer and move:
                myrposr = 3;
                myrposc = 3
                rposr[nb] = myrposr;
                rposc[nb] = myrposc
        ...
        y, v, (hidden, hebb) = net(inputsC, (hidden, hebb), reward_move_time, numstep, BATCHSIZE, hnet)
        print(hebb.shape)
        #for nb in range(BATCHSIZE):
        #    for node in range(params['hs']):
        #        hebbian_trace.append(np.concatenate((np.array([nb, node, numstep]), hebb[nb][node].detach().numpy())))
        ...
        for nb in range(BATCHSIZE):
            ...
            if rposr[nb] == posr[nb] and rposc[nb] == posc[nb]:
                ...
                while lab[posr[nb], posc[nb]] == 1 or (rposr[nb] == posr[nb] and rposc[nb] == posc[nb]):
                    posr[nb] = np.random.randint(1, LABSIZE - 1)
                    posc[nb] = np.random.randint(1, LABSIZE - 1)
            if reward_move_time[nb]+movebuffer < numstep:
    ...
    #print("Move times: ", reward_move_time+movebuffer)
    print("Rewards after move:", sumreward_after)
    #print("Reward Locs", rwloc)
    print("Mean reward (across batch): ", sumreward.mean()+ sumreward_after.mean())
    ...
    print("Done!")
    #np.savetxt("move_all_hebb.csv", hebbian_trace, delimiter=",")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hs", type=int, help="size of the recurrent (hidden) layer", default=20)
