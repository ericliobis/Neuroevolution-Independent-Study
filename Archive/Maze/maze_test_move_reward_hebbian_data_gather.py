...
def train(paramdict):
    ...
    for nb in range(BATCHSIZE):
        ...
        reward_move_time[nb] = nb + movebuffer;#random.sample(range(movebuffer, params['eplen'] - movebuffer), NUMMOVES)
    ...
    for numstep in range(params['eplen']):
        ...
        for nb in range(BATCHSIZE):
            for node in range(params['hs']):
                hebbian_trace.append(np.concatenate((np.array([nb, node, numstep,search[nb]]), hebb[nb][node].detach().numpy())))
        ...
        for nb in range(BATCHSIZE):
            ...
            if moved[nb] and posr[nb] == rposr_old[nb] and posc[nb] == rposc_old[nb]:
                search[nb] = True
