...
#################
#Hebbian_fixed Network#
#################



# class FixedHebbianNetwork(nn.Module):
...
#         # weight decay (float) - weight decay for Adam optimizer - DEFAULT: 0
#         # batch_size (int) - the batch size the network - DEFAULT: 1
#         super(FixedHebbianNetwork, self).__init__()
#         self.isize, self.networksize = isize, len(hsizes)+1
#         self.device = cuda
#         self.lr = lr
#         self.batch_size = batch_size
...
#                 hidden += recurrent_vals.matmul(self.w)#+ torch.mul(self.alpha, hebb))
...
#             #    rec_layer_values = hidden
#         myeta = F.tanh(self.h2mod(rec_layer_values))
#         deltahebb = torch.mul(recurrent_vals, rec_layer_values)
#         myeta = self.modfanout(myeta)
...
#     def initialZeroState(self):
#         return Variable(torch.zeros(self.rec_size), requires_grad=False ).to(self.device)
#     def initialZeroHebb(self):
#         return Variable(torch.zeros(self.rec_size, self.rec_size) , requires_grad=False).to(self.device)
...
#         self.h2mod = torch.nn.Linear(self.rec_size, 1)
#         self.modfanout = torch.nn.Linear(1,self.rec_size)
        #Create the network
        for i in range(self.networksize):
            self.layers.append(torch.nn.Linear(self.inputsizes[i], self.outputsizes[i]))
