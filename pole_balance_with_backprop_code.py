...
seed = 1
env = CartPoleEnv_Rand_Length(seed, True)
env.setTarget(3)
...
TOTALNBINPUTS =  4 + ADDITIONALINPUTS + NBACTIONS
    ...
    for numiter in range(params['nbiter']):
        ...
        for numstep in range(params['eplen']):
            ...
            for nb in range(BATCHSIZE):
                inputs[nb, 0:4] = observation
                reward1 = 1
                if action is not None:
            
                    if numactionschosen[nb] == 0:
                        observation, reward1, done, info = env.step(0)
                    else:
                        observation, reward1, done, info = env.step(1)
                action = 1
                # Previous chosen action
                reward[nb] = reward1
                inputs[nb, 5] = 1.0 # Bias neuron
                inputs[nb, 6] = numstep / params['eplen']
                inputs[nb, 7] = 1.0 * reward[nb]
                inputs[nb, 8 + numactionschosen[nb]] = 1
        ...
        gammaR = params['gr']
        for numstepb in reversed(range(len(rewards))) :
        ...
        if (numiter+1) % params['save_every'] == 0:
            ...
            with open('loss_'+suffix+'.txt', 'w') as thefile:
                for item in all_total_rewards[::10]:
                        thefile.write("%s\n" % item)
            torch.save(net.state_dict(), 'torchmodel2_forMine_'+suffix+'.dat')
            ...
            with open("SCORE_HIST_THEIRCODE_2.csv", "a", newline = '') as my_csv:
                csvWriter = csv.writer(my_csv, delimiter=',')
                print(all_total_rewards)
                csvWriter.writerows(map (lambda x: [x], all_total_rewards))
                all_total_rewards = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eplen", type=int, help="length of episodes", default=1000)
    parser.add_argument("--hs", type=int, help="size of the recurrent (hidden) layer", default=4)
    ...
    parser.add_argument("--nbiter", type=int, help="number of learning cycles", default=500002)
